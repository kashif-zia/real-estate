...
const GridView = ({ side, size, gridType, listSize, mapModal, mapView, relativeSlider, gridBar, video, tabHeader, setMapModal, children, AdvancedSearchShow, infiniteScroll, myList,userProperties }) => {
      ...
      <Container>
        <Row className=" ratio_63">
          ...
          <Col xl={side ? "9" : ""} lg={side ? "8" : ""} className={`${relativeSlider ? "property-grid-3" : "property-grid-2"}  property-grid-slider`}>
            <Header grid={grid} gridDispatch={gridDispatch} userProperties={userProperties} title={"Properties Listing"} mapModal={mapModal} gridBar={gridBar} tabHeader={tabHeader} AdvancedSearchShow={AdvancedSearchShow} value={value} setMapModal={setMapModal} />
            <FilterTag />
            {children}
            <div className={`property-wrapper-grid ${grid.gridStyle ? "list-view" : ""}`}>
              <GridLayout grid={grid} myList={myList} userProperties={userProperties}  value={value} listSize={listSize} relativeSlider={relativeSlider} video={video} gridDispatch={gridDispatch} infiniteScroll={infiniteScroll} />
            </div>
            {infiniteScroll ? (
